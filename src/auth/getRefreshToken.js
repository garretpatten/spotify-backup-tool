...
class SpotifyAuth {
  ...
  async saveRefreshToken(refreshToken) {
    ...
    try {
      ...
      let refreshTokenLineIndex = lines.findIndex((line) =>
  ...
  async testConnection() {
    if (!process.env.SPOTIFY_REFRESH_TOKEN) {
      console.log(
        "❌ No refresh token found. Please run the authentication first.",
      );
      return false;
    }
    ...
    try {
      ...
      console.log(
        `👤 Logged in as: ${me.body.display_name} (${me.body.email})`,
      );
      return true;
    } catch (error) {
