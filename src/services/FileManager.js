...
class FileManager {
  ...
  async savePlaylist(playlistData) {
    try {
      ...
      this.logger.error(
        `Failed to save playlist "${playlistData.name}":`,
        error,
      );
      throw error;
    }
  ...
  async createZipArchive() {
    return new Promise((resolve, reject) => {
      ...
      output.on("close", () => {
        this.logger.info(
          `📦 Archive created: ${zipFileName} (${archive.pointer()} bytes)`,
        );
        resolve(zipPath);
      });
  ...
  async getBackupStats() {
    try {
      ...
      if (await fs.pathExists(publicDir)) {
        ...
        stats.publicPlaylists = publicFiles.filter((file) =>
          file.endsWith(".json"),
        ).length;
      }
      ...
      if (await fs.pathExists(privateDir)) {
        ...
        stats.privatePlaylists = privateFiles.filter((file) =>
          file.endsWith(".json"),
        ).length;
      }
      ...
      for (const file of allFiles) {
        try {
          ...
          stats.totalTracks += playlistData.tracks
          ...
          this.logger.warn(
            `Failed to read playlist file ${file}:`,
            error.message,
          );
        }
      }
