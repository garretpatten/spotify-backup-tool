...
describe("SpotifyApi", () => {
  ...
  describe("initialize", () => {
    it("should authenticate successfully with valid refresh token", async () => {
      ...
      expect(mockSpotifyWebApi.setRefreshToken).toHaveBeenCalledWith(
        "test-refresh-token",
      );
      expect(mockSpotifyWebApi.refreshAccessToken).toHaveBeenCalled();
      expect(mockSpotifyWebApi.setAccessToken).toHaveBeenCalledWith(
        "test-access-token",
      );
    });

    it("should throw error if authentication fails", async () => {
      mockSpotifyWebApi.refreshAccessToken.mockRejectedValue(
  ...
  describe("getAllPlaylists", () => {
    ...
    it("should fetch all playlists with pagination", async () => {
      ...
      const mockPlaylists2 = [{ id: "3", name: "Playlist 3" }];
    ...
    it("should handle API errors gracefully", async () => {
      mockSpotifyWebApi.getUserPlaylists.mockRejectedValue(
        new Error("API Error"),
      );
